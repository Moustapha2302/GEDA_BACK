...
class FinanceController extends Controller
{
    ...
    {
        try {
            $bons = BonCommande::with('fournisseur')->latest()->get();
            // ou sans with() si pas de relation encore
            ...
            Log::error('Erreur getBonsCommande', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
    ...
    public function createBonCommande(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                // ajoute les autres règles
            ]);

            $bon = BonCommande::create([
                'numero'       => 'BC' . date('Y') . '-' . str_pad(BonCommande::count() + 1, 4, '0', STR_PAD_LEFT),
                'objet'        => $validated['objet'],
                'montant'      => $validated['montant'],
                'fournisseur'  => $validated['fournisseur'],
                'statut'       => 'En attente de visa',
                'createur_id'  => $request->user()?->id ?? 1,
            ]);

            return response()->json([
                'message' => 'Bon de commande créé avec succès',
                'bon'     => $bon->load('fournisseur') // si tu veux les détails du fournisseur
            ], 201);
        } catch (\Exception $e) {
            Log::error('Erreur création bon de commande', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
