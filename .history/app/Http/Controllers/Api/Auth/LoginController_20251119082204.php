...
class LoginController extends Controller
{
    ...
    public function loginS02(Request $request)
    {
        ...
        \Log::info('Connexion S02 réussie', [
            'user_id' => $user->id,
            'email' => $user->email,
            'role' => $user->role
        ]);
    ...
    public function loginControleurFinancier(Request $request)
    {
        ...
        $token = $user->createToken('geda-controleur-financier-token', [
    ...
    public function loginChefS02(Request $request)
    {
        ...
        $token = $user->createToken('geda-chef-s02-token', [
            'service:s02',
            'role:chef'
        ])->plainTextToken;
    ...
    public function loginAgentS02(Request $request)
    {
        try {
            ...
            if (!$user) {
                \Log::warning('Agent S02 introuvable', ['email' => $validated['email']]);
                return response()->json([
            ...
            if (!Hash::check($validated['password'], $user->password)) {
                \Log::warning('Mot de passe incorrect pour Agent S02', ['email' => $validated['email']]);
                return response()->json([
                    'message' => 'Mot de passe incorrect'
                ], 401);
            }

            $token = $user->createToken('geda-agent-s02-token', [
                'service:s02',
                'role:agent'
            ])->plainTextToken;
            ...
            \Log::error('Erreur login Agent S02', [
                'error' => $e->getMessage(),
